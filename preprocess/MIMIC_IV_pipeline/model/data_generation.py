...
class Generator():
    ...
    def generate_proc(self):
        ...
        proc[['start_days', 'dummy','start_hours']] = proc['proc_time_from_admit'].str.split(pat=' ',n= -1, expand=True)
    ...
    def generate_labs(self):
        ...
        for labs in tqdm(pd.read_csv("./data/features/preproc_labs.csv.gz", compression='gzip', header=0, index_col=None,chunksize=chunksize)):
            ...
            labs[['start_days', 'dummy','start_hours']] = labs['lab_time_from_admit'].str.split(pat=' ',n= -1, expand=True)
            labs[['start_hours','min','sec']] = labs['start_hours'].str.split(pat=':',n= -1, expand=True)
        ...
        for hid in tqdm(self.hids):
            ...
            if(self.feat_med):
                ...
                if df2.shape[0]==0:
                    ...
                    # modification for error ValueError: could not broadcast input array from shape . into shape .
                    if df2.shape[0]!=val.shape[0]:
                        nb_cop = len(df2) - len(val)
                        line = df2.iloc[-1]
                        new_line = pd.concat([df2]*nb_cop, ignore_index=True)
                        val = pd.concat([val, new_line], ignore_index=True)
